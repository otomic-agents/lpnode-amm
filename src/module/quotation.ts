...
class Quotation {
    ...
    private async reportQuotationError(ammContext: AmmContext, e: any) {
        ...
        logger.info("🪰", body);
        const url =
            "http://n8n.edge-dev.xyz/webhook/bd3ae405-e3df-4f99-b8ec-890391d5dfe9";
        try {
    ...
    public async quotationKeep(item: IBridgeTokenConfigItem) {
        ...
        redisPub
            .publish(item.msmq_name + "_" + item.relay_api_key, quoteCmd)
            .catch((e: any) => {
    ...
    public async asksQuote(ammContext: AmmContext) {
        ...
        if (
            ...
            this.onAskQuoteFailed(
                ammContext,
                "The inputAmount must be greater than 0"
            );
            return;
        }

        const [quoteHash, quoteInfo, errMessage] = await this.quotationItem(
            ammContext
        );
     ...
     * @param sourceObject
     */
    public async price_hedge_fee_price(
        ammContext: AmmContext,
        sourceObject: any
    ) {
        if (ammContext.hedgeEnabled) {
            const hedgeIns = await ammContext.bridgeItem.hedge_info.getHedgeIns();
            const hedgeFeeSymbol = await hedgeIns.getHedgeFeeSymbol();
    ...
    private async min_amount(ammContext: AmmContext, sourceObject: any) {
        ...
        if (ammContext.hedgeEnabled) {
            ...
            const gasTokenPrice = quotationPrice.getGasTokenBuyPrice(ammContext);
            const hedgeIns = await ammContext.bridgeItem.hedge_info.getHedgeIns();
            minHedgeInputNumber = await hedgeIns.getMinHedgeAmount(
                ammContext,
                srcTokenPrice,
                dstTokenPrice,
                gasTokenPrice
    ...
    private async calculate_capacity(ammContext: AmmContext, sourceObject: any) {
        ...
        if (ammContext.hedgeEnabled) {
            if (
                ...
                hedgeCapacity = await (
                    await ammContext.bridgeItem.hedge_info.getHedgeIns()
    ...
    private async resizeFee(
        ...
        const gasResult = await this.calculateRecentGasAverage(
            ammContext.baseInfo.dstChain.id
        );
        ...
        if (want_usdt_percentage.gt(mathlib.bignumber(1))) {
            logger.error(
                "❌ Gas cost exceeds transaction value. Cannot proceed with the transaction."
            );
            throw new Error(
                "Transaction gas cost exceeds the total transaction value. Please increase transaction amount or try again later."
            );
        }
    ...
    private async calculateRecentGasAverage(chainId: number) {
        try {
            const transactions = await ammContextModule
                .find({
            ...
            for (const tx of transactions) {
                try {
                    const nativeTokenPrice = parseFloat(
                        tx.quoteInfo.native_token_usdt_price
                    );
                    const nativeTokenPrecision =
                        tx.baseInfo.dstChain.nativeTokenPrecision;

                    let totalGasForTx = 0;

                    if (tx.dexTradeInfo_in?.rawData) {
                        const gasUsedIn = JSON.parse(
                            tx.dexTradeInfo_in.rawData.transfer_info
                        ).gasUsed;
                        totalGasForTx += parseInt(gasUsedIn);
                    }

                    if (tx.dexTradeInfo_in_confirm?.rawData) {
                        const gasUsedConfirm = JSON.parse(
                            tx.dexTradeInfo_in_confirm.rawData.transfer_info
                        ).gasUsed;
                    ...
                    const gasPrice = transferInfo.effectiveGasPrice
                        ? parseInt(transferInfo.effectiveGasPrice, 16)
                        : 1;
                    const gasUsd =
                        totalGasForTx *
                        gasPrice *
                        Math.pow(10, -nativeTokenPrecision) *
                        nativeTokenPrice;
