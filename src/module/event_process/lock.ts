...
class EventProcessLock extends BaseEventProcess {
  ...
  private getBridgeConfig(ammContext: AmmContext): IBridgeTokenConfigItem {
    ...
    const config: IBridgeTokenConfigItem =
      dataConfig.findItemByStrTokenAndDstToken(srcToken, dstToken);
    if (!config) {
      throw new Error(
  ...
  public getTokenInfoAndChannel(msg: IEVENT_LOCK_QUOTE): string[] {
    const tokenSymbol = eventAdaptor.getTokenSymbolFromEventLock(msg);
    if (tokenSymbol === "") {
